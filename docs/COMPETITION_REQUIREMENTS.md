...
Accepted decision `D-015` and [TECHNICAL_ARCHITECTURE_PROPOSAL.md](./TECHNICAL_ARCHITECTURE_PROPOSAL.md) select the documented minimum competition architecture. Accepted decision `D-016` separately authorizes only the exact six-part compatibility spike in section H of that document; the spike has not been executed. Authorization is not implementation or competition evidence: no CockroachDB client, migration, vector, model, AWS, authentication, or competition-tool integration is implemented by this documentation change.
...
Managed MCP is excluded from the accepted minimum architecture. It may be reconsidered only as an optional read-only, post-final Evidence Agent after a separate security and meaningful-integration review. `ccloud` CLI is not selected as a minimum competition tool. `D-016` authorizes only the six bounded compatibility checks; it does not change any fulfillment status, establish either competition-tool integration, authorize application implementation, or authorize public or production deployment.
...
`D-015` accepts the minimum CockroachDB client/migrations/vector, AWS deployment, model, and two-tool architecture; their live-account, compatibility, and competition evidence remains open and no fulfillment row is evidenced. `D-016` authorizes only the exact six-part compatibility spike, which has not been executed and supplies no competition evidence. Before implementation or submission, explicitly decide and record the `D-106` test harness, `D-107` authentication/actor identity, `D-108` retention/administrative policy, `D-109` eligibility/legal/submission compliance, and `D-110` public license/publication plan. The immutable Agent Skill pin and reviewed invocation boundary, live hosting/access details, telemetry, reset procedure, submission owner/timeline, and applicable official-rules interpretation also remain open validation or implementation artifacts. Application implementation remains separately blocked.
